...
`content-type`レスポンスヘッダーを使用すると、APIで返されるコンテンツのタイプがわかります。さらに、各APIエンドポイントの応答タイプについては、APIリファレンスドキュメントで説明されています。
...
ほとんどのAPI呼び出しでは、`box-request-id`レスポンスヘッダーも返されます。このヘッダーの値を、エラー応答の本文に含まれる`request_id`値と混同しないでください。
